...
public class SHARELoadTest extends CamelTestSupport {
    ...
    public void zipTest() throws Exception {
        ...
        parentContext.addRoutes(new RouteBuilder() {
            ...
            File dir = new File("/data/q/share_random10000_discos");

            @Override
            public void configure() throws Exception {

                errorHandler(deadLetterChannel("file:/data/fail"));

                from("file:" + dir + "?delete=true&include=.*\\.rdf$").to("direct:" + ENDPOINT_PROCESS_FILE);

                from("direct:out").process(e -> {
                    System.out.println("X");
                    final Model m = ModelFactory.createDefaultModel();
                    final StringWriter w = new StringWriter();
                    m.read(e.getIn().getBody(InputStream.class), null, "TURTLE");
                    m.write(w);
                    e.getIn().setBody(w.toString());
                ...
                from("direct:_do_http_op").to("direct:_sanitize_headers").process(e -> System.out.println("YAY"))
